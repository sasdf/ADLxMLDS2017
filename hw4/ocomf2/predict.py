...
print("[*] Loading model")
noiseDim = 128
generator = torch.load(os.path.join(MODEL, 'output', 'Generator-56.pt'))
colorizer = torch.load(os.path.join(MODEL, 'output', 'Colorizer-8.pt'))
generator = generator.train()
colorizer = colorizer.eval()

hairIllum = [0.4, 0.9, 0.6, 0.5, 0.5, 0.3, 0.5, 0.6, 0.35, 0.15, 0.4, 0.7]
...
with open(sys.argv[1]) as f:
    ...
    for l in f:
        ...
        for i, img in enumerate(x):
            ...
            #  rr.append(img)
            io.imsave('samples/sample_%s_%d.jpg' % (id, i+1), img)
