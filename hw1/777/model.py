...
class Model(object):
    ...
    def fit(self, dataset):
        ...
        correct = 0
        total = 0
        for i, (c, x, l, y, n) in enumerate(bar):
            c, x, l, y = [Variable(z).cuda() for z in [c, x, l, y]]

            prob = self.model(c, x, l)
    ...
    def score(self, dataset):
        ...
        et = 0
        for i, (c, x, l, y, n) in enumerate(bar):
            c, x, l, y = [Variable(z, volatile=True).cuda() for z in [c, x, l, y]]
            ...
            for ci, xi, yi, li in zip(c.data, pred, y.squeeze(-1).data.cpu().numpy(), l.data.cpu().numpy()):
    ...
    def predict(self, dataset):
        ...
        r = []
        for i, (c, x, l, y, n) in enumerate(bar):
            c, x, l = [Variable(z, volatile=True).cuda() for z in [c, x, l]]

            prob = self.model(c, x, l)

            pred = torch.max(prob.data, 2)[1]
            for ci, xi, yi, li, ni in zip(c.data, pred, y.squeeze(-1).numpy(), l.data.cpu().numpy(), n):
                ci, xi, yi = ci[:li].tolist(), xi[:li].tolist(), yi[:li].tolist()
